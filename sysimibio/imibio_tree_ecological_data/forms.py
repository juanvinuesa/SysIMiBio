...
class TreeForm(forms.ModelForm):
    ...
    def clean(self):  # todo buscar una forma de generar la latitud a partir de la distancia
...
class FieldForm(forms.ModelForm):
    ...
    def clean(self):
        ...
        if coordinator in staff:
            raise ValidationError("Porfavor desmarque coordinador del campo acompañantes")
        ...
        if coordinator_staff_date_instance:
            raise ValidationError("Staff o coordinador ya registrados en otro campo en el mismo dia y hora")
...
class PermanentParcelForm(forms.ModelForm):
    plot_choices = forms.MultipleChoiceField(
        choices=PLOT_CHOICES,
        widget=forms.CheckboxSelectMultiple(),# initial=[c[0] for c in PLOT_CHOICES],
        label=("Tipo de parcela")
                                                )

    class Meta:
        model = PermanentParcel
        exclude = ('created_at', 'plot_type',)
        include = ('plot_choices',)
        widgets = {
            'geom': LeafletWidget(),
        }
    ...
    def clean(self):
        ...
        plot_choices = cleaned_data.get('plot_choices')
        if not(len(plot_choices) <= len(cadastral_parcel.split(","))):
            raise ValidationError(
                "Ingresar una nomenclatura correcta separada por coma Ej '123104000,110994000'")
        return cleaned_data
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.helper.layout = Layout(
            ...
            Row(
                ...
                Column('plot_choices', css_class='form-group col-md-6 mb-0'),
                css_class='form-row'
            ),
            Row(
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.helper.layout = Layout(
            ...
            Row(
                ...
                Column('sociological_classification', css_class='form-group col-md-4 mb-0'),
