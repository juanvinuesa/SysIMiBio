...
from sysimibio.imibio_tree_ecological_data.validators import validate_date, validate_temperature, validate_humidity, \
    validate_lat, validate_lon, tree_height_validation, tree_dap_validation, validate_dist_x, validate_dist_y
...
class PermanentParcel(models.Model):  # todo change to Permanent Plot?
    ...
    plot_type = models.CharField(verbose_name='Tipo de parcela',  max_length=10, default="Fiscal")
    ...
    def entry_saved_data(self):  # todo cambiar el nombre y Juan(revisar metodos en models)
        return json.loads(self.plot_type)
...
class Tree(models.Model):
    ...
    field = models.ForeignKey('FieldWork', on_delete=models.CASCADE, verbose_name="Campo")
    ...
    latitude = models.FloatField(verbose_name='Eje Y', validators=[validate_dist_y],
                                 help_text="Informar la distancia en el y (N-S) en metros")
    longitude = models.FloatField(verbose_name='Eje X', validators=[validate_dist_x],
    ...
    has_herbarium = models.BooleanField(verbose_name="Tiene material en herbario?", default=False)
    herbarium_info = models.TextField(verbose_name="Numero del material de herbario", blank=True)

    @property
    def tree_id(self):
        tree_number_str = str(self.tree_number)
        if self.tree_number < 10:
            tree_number_str = "0" + tree_number_str  # TODO buscar una frma mas eficiente
...
class TreeMeasurement(models.Model):
    ...
    tree_height = models.FloatField(verbose_name='Altura del árbol', help_text='m',
                                    validators=[tree_height_validation], blank=True)
    picture = models.ForeignKey(Pictures, on_delete=models.CASCADE, blank=True, null=True)
    phytosanitary_status = models.IntegerField(verbose_name='Estado fitosanitario',
                                            choices=PHYTOSANITARY_STATUS_CHOICES,
                                            default=BUENO)
    sociological_classification = models.IntegerField(verbose_name='Clasificación sociologica',
                                                   choices=SOCIOLOGICAL_CLASSIFICATION_CHOICES,
                                                   default=PREDOMINANTE)
    liana_cover = models.IntegerField(verbose_name="Cobertura de lianas",
                                      choices=LIANA_CHOICES,
                                      default=SOLO_FUSTE)
    obs = models.TextField(verbose_name="Observaciones", blank=True)
    created_at = models.DateTimeField(verbose_name='Fecha creación', auto_now_add=True, null=True)
